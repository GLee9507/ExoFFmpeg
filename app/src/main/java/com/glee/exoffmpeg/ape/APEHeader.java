...
public class APEHeader {
    ...
    private APEHeader(final ExtractorInputWrapper input, int version) {
        this.input = input;
        this.version = version;
    }

    public static APEHeader sniff(ExtractorInputWrapper input) throws IOException, InterruptedException {
        try {
            if (!input.readString(4, "US-ASCII").equals("MAC "))
                return new APEHeader(input, input.readUnsignedShort());
            else
                return null;
        } finally {
...
//        }
    ...
    protected void AnalyzeCurrent(APEFileInfo m_APEFileInfo) throws IOException, InterruptedException {
        ...
        if ((m_APEFileInfo.spAPEDescriptor.nDescriptorBytes - APEDescriptor.APE_DESCRIPTOR_BYTES) > 0)
            input.skip((int) (m_APEFileInfo.spAPEDescriptor.nDescriptorBytes - APEDescriptor.APE_DESCRIPTOR_BYTES));
        ...
        if ((m_APEFileInfo.spAPEDescriptor.nHeaderBytes - APEHeaderNew.APE_HEADER_BYTES) > 0)
            input.skip((int) (m_APEFileInfo.spAPEDescriptor.nHeaderBytes - APEHeaderNew.APE_HEADER_BYTES));
...
//            m_APEFileInfo.spSeekByteTable[i] = m_pIO.readIntBack(); TODO 未实现
    ...
    protected void AnalyzeOld(APEFileInfo m_APEFileInfo) throws IOException, InterruptedException {
        ...
        if ((header.nFormatFlags & MAC_FORMAT_FLAG_HAS_PEAK_LEVEL) > 0)
            nPeakLevel = (int) input.readUnsignedInt();

        if ((header.nFormatFlags & MAC_FORMAT_FLAG_HAS_SEEK_ELEMENTS) > 0)
            m_APEFileInfo.nSeekTableElements = (int) input.readUnsignedInt();
        else
        ...
        m_APEFileInfo.spSeekByteTable = new int[m_APEFileInfo.nSeekTableElements];
        for (int i = 0; i < m_APEFileInfo.nSeekTableElements; i++)
            m_APEFileInfo.spSeekByteTable[i] = (int) input.readUnsignedInt();
